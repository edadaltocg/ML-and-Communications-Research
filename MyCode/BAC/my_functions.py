...
def get_class(choice, train_ps):
    classes = np.array(range(np.size(train_ps))) 
    M = to_categorical(classes, num_classes=np.size(classes), dtype='float32')
    return M[choice]
    
def TensorBACconcat(x, q, train_ps):
    choice = np.random.randint(0, np.size(train_ps))
    p = train_ps[choice]
    coded_p = K.constant(get_class(choice, train_ps))
    aux = K.ones((K.shape(x)[0], 1))
    repeated = tf.multiply(aux, coded_p)
    noise = K.random_uniform(shape=K.shape(x), minval=0.0, maxval=1.0)    
    decision = (1-x)*p + q*x
    flip = K.less(noise, decision)
    result =  tf.math.add(tf.multiply(K.cast(flip, dtype='float32'), 1.0), x)%2
    return K.concatenate((result, repeated), axis=1)
